...
static slabclass_t slabclass[MAX_NUMBER_OF_SLAB_CLASSES]; /* 结构体数组,数组大小201 */
...
unsigned int slabs_clsid(const size_t size) {
    ...
    if (size == 0)
        return 0;
    while (size > slabclass[res].size)   /* 找一个最小大于size classid.数组下标 */
...
void slabs_init(const size_t limit, const double factor, const bool prealloc) {
    ...
    unsigned int size = sizeof(item) + settings.chunk_size;/* 默认48, item自身48=96 */
    ...
    memset(slabclass, 0, sizeof(slabclass)); /* 设置默认值 */
    ...
    while (++i < POWER_LARGEST && size <= settings.item_size_max / factor) {
        ...
        slabclass[i].size = size; /* 不同的size标记为不同的class */
    ...
    if (settings.verbose > 1) {
        fprintf(stderr, "slab class %3d: chunk size %9u perslab %7u\n",
                i, slabclass[i].size, slabclass[i].perslab);
    } /* 初始化不同class类型的 slabclass里的size perslab */
...
static int grow_slab_list (const unsigned int id) {
    ...
    } /* 动态数组来管理slabclass下 slab_list, 2倍来扩容 */
    return 1;
}
...
static void split_slab_page_into_freelist(char *ptr, const unsigned int id) {
    slabclass_t *p = &slabclass[id];
    int x;
    for (x = 0; x < p->perslab; x++) { /* 新的slab分割成空闲的item, 双向链表格式,头地址放在slots */
        do_slabs_free(ptr, 0, id);     
        ptr += p->size; /* ptr指向下一个item.新分配的slab,把里面的item放到slot双向链表中 */
    }
}

static int do_slabs_newslab(const unsigned int id) {
    slabclass_t *p = &slabclass[id]; /* 获取指定slabclass */
    ...
    split_slab_page_into_freelist(ptr, id); /* 加入slab slots列表 */

    p->slab_list[p->slabs++] = ptr; /* slab_list二级指针(指针数组) key:当前分配个数 value:slab内存地址 */
    mem_malloced += len;
...
static void *do_slabs_alloc(const size_t size, unsigned int id) {
    ...
    if (! (p->sl_curr != 0 || do_slabs_newslab(id) != 0)) {
        ...
        ret = NULL;
    } else if (p->sl_curr != 0) { /* sl_curr 空闲item总数 */
        /* return off our freelist */
        it = (item *)p->slots;  /* 获取slots指向第一个item, O(1) */
        p->slots = it->next;    /* slots指向下一个 */
        if (it->next) it->next->prev = 0;
        p->sl_curr--;           /* 分配了一个item，空闲数减1 */
        ret = (void *)it;
    }
...
static void do_slabs_free(void *ptr, const size_t size, unsigned int id) {
    ...
    it = (item *)ptr;   /* 第一次分配slab, ptr是指向slab地址.这里格式化成item就行分配 */
    it->it_flags |= ITEM_SLABBED;
    it->prev = 0;
    it->next = p->slots;
    if (it->next) it->next->prev = it;
    p->slots = it; /* 新分配slab加入到slots双向链表中 */
