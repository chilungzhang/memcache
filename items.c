 ...
 * flags   - key flags
 * nbytes  - Number of bytes to hold value and addition CRLF terminator (value长度包括结尾符)
...
static size_t item_make_header(const uint8_t nkey, const int flags, const int nbytes,
    ...
    *nsuffix = (uint8_t) snprintf(suffix, 40, " %d %d\r\n", flags, nbytes - 2);
    return sizeof(item) + nkey + *nsuffix + nbytes; /* nsuffix作用? item固定大小, key长度, value长度 */
}
...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    item *it = NULL;
    char suffix[40];
    size_t ntotal = item_make_header(nkey + 1, flags, nbytes, suffix, &nsuffix); /* items总长度 */
    if (settings.use_cas) {
        ntotal += sizeof(uint64_t);
    }

    unsigned int id = slabs_clsid(ntotal); /* 根据item总长度,选择slabs classid */
    if (id == 0)
        return 0;

    mutex_lock(&cache_lock); /* 加锁 */
    ...
    search = tails[id];  /* tails全局数组,item指针元素 */
    ...
    if (!tried_alloc && (tries == 0 || search == NULL))
        it = slabs_alloc(ntotal, id); /* 分配slabs */
    ...
    it->nkey = nkey;
    it->nbytes = nbytes;
    memcpy(ITEM_key(it), key, nkey);    /* key存储 */
    it->exptime = exptime;
    memcpy(ITEM_suffix(it), suffix, (size_t)nsuffix);   /* nsuffix存储 */
    it->nsuffix = nsuffix;
    return it;
}
...
static void item_link_q(item *it) { /* item is the new head */
    ...
    head = &heads[it->slabs_clsid]; /* heads,tails全局数组，数组元素是slabs_classid的指针 */
...
int do_item_link(item *it, const uint32_t hv) {
    ...
    assoc_insert(it, hv); /* item加入到hash表中 */
    item_link_q(it);
